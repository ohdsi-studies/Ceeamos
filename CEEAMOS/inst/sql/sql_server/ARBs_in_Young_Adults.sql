...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (40235485,1351557,1346686,1347384,1367500,40226742,1317640,1308842)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (40235485,1351557,1346686,1347384,1367500,40226742,1317640,1308842)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1335471,1340128,1395058,1341927,1363749,974166,978555,1308216,907013,1310756,1373225,1331235,1334456,1342439)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1335471,1340128,1395058,1341927,1363749,974166,978555,1308216,907013,1310756,1373225,1331235,1334456,1342439)
  and c.invalid_reason is null
...
WHERE (YEAR(C.drug_exposure_start_date) - P.year_of_birth >= 18 and YEAR(C.drug_exposure_start_date) - P.year_of_birth <= 45)
AND C.ordinal = 1
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= DATEADD(day,7,P.START_DATE)
GROUP BY p.person_id, p.event_id
